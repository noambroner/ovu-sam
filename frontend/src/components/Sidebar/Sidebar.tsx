...
const SIDEBAR_STATE_KEY = 'sam_sidebar_state';
...
export const Sidebar = ({ menuItems, currentPath, language, theme, onNavigate }: SidebarProps) => {
  ...
  const getLabel = (item: MenuItem) => {
    if (language === 'he') return item.label;
  ...
  const sidebarTitle = language === 'he' ? 'תפריט ראשי' : language === 'ar' ? 'القائمة الرئيسية' : 'Main Menu';
